/*
  ...
  Monophonic - Last note, with 20 notes in buffer

  Note CV output (1V/octave, 88 keys if amplified, xx keys if not)
  Velocity CV output (0 to 5V)

  Pitch bend CV output (0 to 1V, witch 0.5 central possition)
  Control Change CV out (0 to 5V, CC1-4)
...
// DAC pin mappings
#define PITCH_CH        UNASSIGNED
#define BEND_CH         UNASSIGNED
#define VELO_CH         MCP4728_CHANNEL_A
#define CC1_CH          MCP4728_CHANNEL_B
#define CC2_CH          MCP4728_CHANNEL_C
#define CC3_CH          MCP4728_CHANNEL_D
...
void callbackNoteOn(byte channel, byte pitch, byte velocity) {
  ...
  if (note >= 0 && note < noteRange) {
    ...
    if(currentIndex < (bufferSize - 1)) currentIndex++;
...
void callbackControlChange(byte channel, byte number, byte value) {
  ...
  if (number == 1 && CC1_CH != UNASSIGNED) activeChannel = CC1_CH;
  else if (number == 2 && CC2_CH != UNASSIGNED) activeChannel = CC2_CH;
  else if (number == 3 && CC3_CH != UNASSIGNED) activeChannel = CC3_CH;
  else if (number == 4 && CC4_CH != UNASSIGNED) activeChannel = CC4_CH;
...
void callbackClock(void) {
  now = millis();
  if (now > clockTimeout + 300) clockCount = 0; // Prevents Clock from starting in between quarter notes after clock is restarted!
  clockTimeout = now;
  ...
  clockCount++;
  if (clockCount == 24) {  // MIDI timing clock sends 24 pulses per quarter note.  Sent pulse only once every 24 pulses
    clockCount = 0;
  }
}
...
void setup() {
  ...
  const float pitchFactor = 4095.0f / (noteRange - 1);
  ...
  if (GATE_PIN != UNASSIGNED || TRIGGER_PIN != UNASSIGNED || PITCH_CH != UNASSIGNED || VELO_CH != UNASSIGNED) {
    ...
    // Pitch
    if (PITCH_CH != UNASSIGNED) mcp.setChannelValue(PITCH_CH, 0);
    // Velocity
    if (VELO_CH != UNASSIGNED) mcp.setChannelValue(VELO_CH, 0);
  }
  ...
  if (CC1_CH != UNASSIGNED || CC2_CH != UNASSIGNED || CC3_CH != UNASSIGNED || CC4_CH != UNASSIGNED) {
    ...
    if (CC1_CH != UNASSIGNED) mcp.setChannelValue(CC1_CH, 0);
    if (CC2_CH != UNASSIGNED) mcp.setChannelValue(CC2_CH, 0);
    if (CC3_CH != UNASSIGNED) mcp.setChannelValue(CC3_CH, 0);
    if (CC4_CH != UNASSIGNED) mcp.setChannelValue(CC4_CH, 0);
  }
